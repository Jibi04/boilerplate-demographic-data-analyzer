...
def calculate_demographic_data(print_data=True):
    ...
    rich_in_country = df[df['salary'] == '>50K']['native-country'].value_counts() 
    country_population = df.groupby('native-country').size()
    
    highest_earning_countries = round((rich_in_country / country_population) * 100, 1).sort_values(ascending=False)
    
    highest_earning_country = highest_earning_countries.idxmax()
    highest_earning_country_percentage = max(highest_earning_countries)
